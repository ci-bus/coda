...
?>	
			...
			<section class="generic-banner">				
				<div class="container">
					...
					<a href="prueba.php?id=<?php echo $idCarrera;?>" class="primary-btn text-uppercase">VOLVER</a>
					<a href="acceso.php?id=<?php echo $idCarrera;?>"" class="primary-btn text-uppercase">ACCEDER</a>
				</div>
			</section>
			<div class="section-top-border">
						<h3 class="mb-30">DOCUMENTACI&Oacute;N PREVIA:</h3>
						<?php
						$previa = mysql_query("SELECT titulo,archivo FROM infoprevia WHERE idcarreras='$idCarrera' AND activo=1 ORDER BY idInfo ASC");
						if(mysql_num_rows($previa)>0)
							...
							while($fila=mysql_fetch_array($previa))
								...
								echo '<tr class="'.$classcss.'"><td><a href="http://codea.es/'.$archivo.'" target="_blank">'.$titulo.'</a></td></tr>';
								$par++;
								}
							echo '</table>';
							}
			...
			<div class="section-top-border">
						...
						<?php
						$previa = mysql_query("SELECT titulo,archivo FROM infocarrera WHERE idcarreras='$idCarrera' AND activo=1 ORDER BY idInfo ASC");
						if(mysql_num_rows($previa)>0)
							{
								$par=0;
							echo '<table width="100%">';
			...
			<div class="section-top-border">
						...
						<?php
						$previa = mysql_query("SELECT titulo,archivo FROM infocomisarios WHERE idcarreras='$idCarrera' AND activo=1 ORDER BY idInfo ASC");
						if(mysql_num_rows($previa)>0)
							{
								$par=0;
							echo '<table width="100%">';
			...
			<div class="section-top-border">
						...
						<?php
						$previa = mysql_query("SELECT titulo,archivo FROM infodireccion WHERE idcarreras='$idCarrera' AND activo=1 ORDER BY idInfo ASC");
						if(mysql_num_rows($previa)>0)
							{
								$par=0;
							echo '<table width="100%">';
