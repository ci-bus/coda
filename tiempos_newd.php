		...
		<body>
			...
			<section class="generic-banner">				
				<div class="container">
					...
					<a href="clas_final_newb.php?id=<?php echo $idCarrera;?>&newBD=true&campeonato=0&copa=0&clase=0&grupo=0&agrupacion=0&categoria=0" class="primary-btn text-uppercase selected">CLASIFICACI&Oacute;N FINAL</a>
				</div>
			</section>	
...
<h3 class="mb-30">TRAMOS DISPONIBLES</h3>
										...
										while($fila2=mysql_fetch_array($secciones))
											...
											if(mysql_num_rows($secciones)>0)
												...
												while($fila3=mysql_fetch_array($mangas))
													...
													$estado = $fila3['estado'];
													$long = $fila3['longitud'];
													$inter = mysql_query("SELECT id FROM abc_57os_ca_manga_control_horario WHERE id_ca_manga='$idmanga'");
													$porsalir=0;
													$enpista=0;
													$enmeta = 0;
													$sql_pista = mysql_query("SELECT m.tipo AS tipo_manga,m.numero AS num_manga,com.dorsal,pi.nombre AS piloto_nombre,
													...
													if(mysql_num_rows($sql_pista)>0)
															{
															while($fila=mysql_fetch_array($sql_pista))
																{
																$dorsal = $fila['dorsal'];
															...
															AND t.dorsal='$dorsal' AND t.id_ca_manga='$idmanga'");//COMPRUEBO Q TENGA 2  TIEMPOS salida y llegada
																	if(mysql_num_rows($comp)==1)//0 PÔR SALIR
																		{
																		$enpista++;
																		}
																	else if(mysql_num_rows($comp)==0)//0 PÔR SALIR
																		{
																		$porsalir++;
																		}
																	else{
																		while($rows = mysql_fetch_array($comp))
																			{
																			$orden = $rows['orden'];
																			if($orden==10)
																				$enmeta++;
													...
													$pen = mysql_query("SELECT a.motivo FROM abc_57os_ca_abandono a	WHERE a.id_ca_manga='$idmanga'");
													$abandonos = mysql_num_rows($pen);
														if(mysql_num_rows($inter)>2)
														{
															echo '<tr class="manga '.$classcss.'"><td class="man_pd">
															<p><a href="manga_newd.php?id='.$idCarrera.'&idetapa='.$idetapa.'&idmanga='.$idmanga.'&idseccion='.$idseccion.'&newBD=true">'.$descripcion.'</a></p>';
															...
															echo '<tr class="manga '.$classcss.'"><td class="man_pd">
															<p><a href="manga_newd.php?id='.$idCarrera.'&idetapa='.$idetapa.'&idmanga='.$idmanga.'&idseccion='.$idseccion.'&campeonato=0&newBD=true&copa=0">'.$descripcion.'</a></p>';
															echo '<p class="margen_p_coches"><span class="span_coches">'.$porsalir.'<img src="img/coche2.png" width="30px" class="margen_coches"></span><span class="span_coches">'.$enpista.'<img src="img/coche3.png" width="30px" class="margen_coches"></span><span class="span_coches">'.$enmeta.'<img src="img/coche1.png" width="30px" class="margen_coches"></span><span class="span_coches">'.$abandonos.'<img src="img/coche4.png" width="30px" class="margen_coches"></span></p>';
															echo '</td><td></td>
															<td>'.$hora.' H</td><td>'.($long/1000).' Kms</td><td><img src="img/'.estado($estado).'" width="15px"></td></tr>';
														}
			...
			<div class="section-top-border">
						...
						<div class="progress-table-wrap">
							...
							<?php
							$sql=mysql_query("SELECT * FROM web_ult_hora WHERE idcarrera='$idCarrera' ORDER BY id DESC");
