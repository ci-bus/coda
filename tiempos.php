		...
		<body>
			...
			<section class="generic-banner">				
				<div class="container">
					...
					<a href="inscritos.php?id=<?php echo $idCarrera;?>" class="primary-btn text-uppercase">INSCRITOS</a>
						...
						<div class="progress-table-wrap">
							<div class="progress-table">
								<div class="table-head">
									...
									<div class="country">Longitud</div>
									<div class="visit">Hora Comienzo</div>
									<div class="percentage">Estado</div>
								</div>
								<?php
									$dbQuery = "SELECT * FROM mangas WHERE idcarreras = '$idCarrera' ORDER BY orden ASC";
$resultado = mysql_query($dbQuery) or print "No se pudo acceder al contenido de los tiempos online.";
//SELECT COUNT(*) FROM controles_intermedios WHERE idcarreras = '$idCarrera' AND (idmangas IS NOT NULL);//Contar los registros
$dbQueryT = "SELECT * FROM controles_intermedios WHERE idcarreras = '$idCarrera' AND (idmangas IS NOT NULL) ORDER BY idmangas ASC, orden ASC";//Rrefiero hacer una consulta y ver si hay filas, ya que con COUNT(*) tendría que leer el registro.
$resultadoT = mysql_query($dbQueryT) or print "No se pudo acceder al contenido de los tiempos online.";
$par=0;
if(mysql_num_rows($resultado==0))
	echo "ERROR ACCEDIENDO";
else{
while($fila=mysql_fetch_array($resultado)){
	...
	$hora_comienzo = $fila["hora_teorica_salida_primero"];
	$orden = $fila["orden"];
	$est = $fila["estado"];
 if($par%2==0){
		$classcss="filapar";
	...
	echo '<a href="manga.php?idmanga='.$idManga.'&id='.$idcarreras.'&orden='.$orden.'"><div class="table-row '.$classcss.'">
			<div class="serial"><a href="manga.php?idmanga='.$idManga.'&id='.$idcarreras.'&orden='.$orden.'">'.$nombre.'</a></div>
				<div class="country">'.$longitud.'</div>
				<div class="visit">'.$hora_comienzo.'</div>
				<div class="percentage">
					<div class="progress">'.$est.'</div>
				</div>
		</div></a>';
		$par++;
	}
