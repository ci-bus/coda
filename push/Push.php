...
class Push {
    ...
    private $user  = 'web2020';
    private $password   = "Kp!vt750";
    private $database  = "web2020";    
	...
	public function listNotificationUser($user){
		...
		$sqlQuery = "SELECT * FROM ".$this->notifTable." WHERE notif_loop > 0 AND username='$user'";
		return  $this->getData($sqlQuery);
	}	
	...
	public function loginUsers($username){
		...
		$sqlQuery = "SELECT id as userid, username, password FROM ".$this->userTable." WHERE username='$username'";
        return  $this->getData($sqlQuery);
	}	
	...
	public function updateNotification($id, $nextTime) {		
		...
		$sqlUpdate = "UPDATE ".$this->notifTable." SET notif_loop = 0 WHERE id='$id'";
