		...
		while($fila=mysql_fetch_array($resultado))
			...
			$abandono = mysql_query("SELECT id FROM abc_57os_ca_abandono WHERE id_ca_manga='$idManga' AND dorsal='$dorsal'");
										...
										while($fil=mysql_fetch_array($comp))
											...
											if($tipo==1 && $orden==10)
												...
												//if($dorsal==3){
													...
													//}
												//echo $dorsal."-".milisegundos_a_tiempo($tmanga[$cont])."<>";
												}
											}
								}
							else{
				...
				//$cont=1;
					while($num_manga !=1)
						...
						INNER JOIN abc_57os_ca_manga_control_horario ch ON ch.id=t.id_ca_manga_control_horario 
						INNER JOIN abc_57os_ca_manga m ON ch.id_ca_manga=m.id 
						WHERE t.dorsal='$dorsal' AND t.id_ca_manga='$mimanga' AND m.tipo=1");
			...
			$c_pen = mysql_query("SELECT tiempo FROM abc_57os_ca_penalizacion WHERE id_ca_manga='$idManga' AND dorsal='$dorsal'");
				...
				$t_p=segundos_a_milisegundos($pen);
					...
					if($dorsal==27 AND $idManga==132) //ÑAPA ERMITAS
						$tiempo=0;
...
<table border="0" width="100%" id="tab_tem" class="des_pe1">
	...
	if($tipo_manga==0 AND $num_manga==1) //LA MANGA ES LA 1 Y NO PUNTUA
		...
		foreach ($ordenar as $key => $row) {
		$aux[$key] = milisegundos_a_tiempo($row['acu_hasta']);
	}
	array_multisort($aux, SORT_ASC, $ordenar);
	$pos=1;
	foreach ($ordenar as $key => $row) {
	...
	//////////////////////////////Y TERMINAMOS CALCULO DE POSICIOnES/////////////////////////////////////
						if($pos==1){ //EL MEJOR TIEMPO......SIN DUDA ES EL PRIMErOO ;-)
							$mejor_tiempo_acumulado = $acu_hasta;
							$tiempo_acu_anterior = $acu_hasta;
						}
		...
		if(strlen($mejor_tiempo_acumulado)>9){
				$dif_acu_primero = milisegundos_a_tiempo(tiempo_a_milisegundos($acu_hasta) - tiempo_a_milisegundos($mejor_tiempo_acumulado));
				$dif_acu_anterior = milisegundos_a_tiempo(tiempo_a_milisegundos($acu_hasta) - tiempo_a_milisegundos($tiempo_acu_anterior));
				}
			else{
				$dif_acu_primero = milisegundos_a_tiempo(tiempo_a_milisegundos("00:".$acu_hasta) - tiempo_a_milisegundos("00:".$mejor_tiempo_acumulado));
				$dif_acu_anterior = milisegundos_a_tiempo(tiempo_a_milisegundos("00:".$acu_hasta) - tiempo_a_milisegundos("00:".$tiempo_acu_anterior));
			}
	...
	echo '<td class="cla">'.escudo($row['vehiculo']).'</td><td><span class="veh mini3">'.$row['vehiculo'].' '.$row['modelo'].'</span>
		...
		<td class="tie centro">';
			if($pos!=1)
				echo '<p class="anterior cursiva nomargen mini2">+'.$dif_acu_anterior.'</p><p class="negrita nomargen mini2">'.$acu_hasta.'</p><p class="cursiva nomargen mini2">+'.$dif_acu_primero.'</p>';
			else
				echo '<p class="negrita nomargen mini2">'.$acu_hasta.'</p>';
			if($row['pen_hasta']!=0)
...
//unset($tiempos);
	...
	echo '<td class="cla">'.escudo($row['vehiculo']).'</td><td><span class="veh">'.$row['vehiculo'].' '.$row['modelo'].'</span>';
		...
		<td class="tie centro">';
			if($pos!=1)
				echo '<p class="anterior cursiva nomargen">+'.$dif_acu_anterior.'</p><p class="negrita nomargen">'.$acu_hasta.'</p><p class="cursiva nomargen">+'.$dif_acu_primero.'</p>';
			else
				echo '<p class="negrita nomargen">'.$acu_hasta.'</p>';
			if($row['pen_hasta']!=0)
...
<table border="0" width="100%" id="tab_tem">
				...
				<tr>
					...
					<tbody>
					<?php
					$pen = mysql_query("SELECT veh.marca,veh.modelo,pi.nombre AS piloto,con.nombre AS concursante,c.dorsal,a.motivo FROM abc_57os_ca_abandono a
