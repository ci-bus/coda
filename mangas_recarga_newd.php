...
if($copa=='0')
	...
	$max_vuelta=0;
	/*SABER EL MAXIMO DE VUELTAS QUE HAY EN CARRERA
	
	if(mysql_num_rows($resultado)>0)
		{
		while($filb=mysql_fetch_array($resultado))
			{
			$dorsal = $filb['dorsal'];
	$comp2 = mysql_query("SELECT ch.orden AS orden,t.tiempo,m.tipo FROM abc_57os_ca_tiempo t 
			...
			$abandono2 = mysql_query("SELECT id FROM abc_57os_ca_abandono WHERE id_ca_manga='$idManga' AND dorsal='$dorsal'");
							if(mysql_num_rows($abandono2)==0)
								{
								$aban=0;
								$tipo=0;
									if(mysql_num_rows($comp2)>=2)//MIN 2 TIEMPOS, SALIDA - LLEGADA
										{
										if($vuelta>$max_vuelta)
											$max_vuelta = $vuelta;
										$vuelta = 0;
										while($file=mysql_fetch_array($comp2))
											{
											$tipo = $file['tipo'];
											$orden = $file['orden'];
											if($tipo!=0 && $orden==10)
												...
												$cont++;
												$vuelta++;
												//AQUI FALTA BUSCAR MEJOR TIEMPO DE VUEKTA Y DIFERENCIAR SHAKEDOWN
												}
											}
										}
								}
...
if(mysql_num_rows($resultado)>0)
		...
		while($fila=mysql_fetch_array($resultado))
			...
			INNER JOIN abc_57os_ca_manga_control_horario ch ON ch.id=t.id_ca_manga_control_horario 
			INNER JOIN abc_57os_ca_manga m ON ch.id_ca_manga=m.id 
			WHERE t.dorsal='$dorsal' AND t.id_ca_manga='$idManga' AND m.tipo!=0");
									...
									if(mysql_num_rows($comp)>=2)//MIN 2 TIEMPOS, SALIDA - LLEGADA
										...
										while($fil=mysql_fetch_array($comp))
											...
											if($tipo!=0 && $orden==10)
												...
												$cont++;
												$vuelta++;
												$v[$vuelta] = tiempo_a_milisegundos($t_l)-tiempo_a_milisegundos($t_s);
												if($v[$vuelta]<$mejor_vuelta){
													$mejor_vuelta=$v[$vuelta];
													$tmanga[$cont] = $mejor_vuelta;
													}
												else
...
<table border="0" width="100%" id="tab_tem" class="des_pe1">
				...
				<tr>
					...
					<tbody>
				<?php
	if(($tipo_manga==0 OR $tipo_manga==2)AND $num_manga==1) //LA MANGA ES LA 1 Y NO PUNTUA
...
?>				
	...
	<tr>
		...
		<?php
			if($no_concursante==1)
				...
				if($tipo_manga==2)
					echo "<th>P.</th><th>N.</th><th colspan='2'>Concursante</th><th>Equipo</th><th colspan='2'>Vehiculo<br>Grupo/Clase</th><th colspan='".($max_vuelta+1)."' class='centro'>PASADA</th><th class='centro tie'><p class='anterior cursiva nomargen'>Anterior</p><p class='negrita nomargen'>Tiempo</p><p class='anterior cursiva nomargen'>Primero</p><p><span class='penalizaciones nomargen'>Penalizacion</span></th>";
				else
	...
	foreach ($ordenar as $key => $row) {
			if($par%2==0)
				...
				$classcss="filaimpar";
		//////POSICIONES GRUPOS Y CLASES///////////SIN MAS COJONES HAY Q RECORRERLO ENTERO////////////////////////
//echo $row['dorsal']."-".$row['tiempo']."--".milisegundos_a_tiempo($row['v3'])."<br>";	
				foreach ($ordenar as $key2 => $row2) {
						$aux2[$key2] = $row2['tiempo'];
					}
					array_multisort($aux2, SORT_ASC, $ordenar);
	...
	if($tipo_manga==2){ //SHAKeDOWN
	echo "<td>";
	for($j=1;$j<=$max_vuelta;$j++){
		$vue = "v".$j;
		if(milisegundos_a_tiempo($row[$vue])==$row['tiempo'])
			echo "<td class='tie'><span class='verde negrita'>".milisegundos_a_tiempo($row[$vue])."</span></td>";
		else
			echo "<td class='tie'>".milisegundos_a_tiempo($row[$vue])."</td>";
		}
	echo "</td>";
	}
		if($pos!=1)
...
//unset($tiempos);
	...
	<?php
	//echo $num_manga;
	if($tipo_manga!=2 AND $num_manga==1){
		?>
		</td>
