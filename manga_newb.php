...
?>	
	...
	<head>
<META HTTP-EQUIV="Refresh" CONTENT="60;URL=<?php echo $enlace_actual;?>">
...
WHERE ca.id_ca_carrera='$idCarrera'";
	...
	if($copa=='0'){
				...
				echo "<option value='manga_newb.php?id=".$idCarrera."&idetapa=".$idetapa."&idmanga=".$idManga."&idseccion=".$idseccion."&newBD=true&campeonato=0&copa=0'>TODAS LAS COPAS</option>";
	   }
	while($row=mysql_fetch_array($copas))
		...
		$filtroDesc = str_replace('ESPAÃ‘A', 'ESPAÑA', $filtroDesc);
			...
			echo "<option value='manga_newb.php?id=".$idCarrera."&idetapa=".$idetapa."&idmanga=".$idManga."&idseccion=".$idseccion."&newBD=true&campeonato=0&copa=".$filtroCopa."'>".$filtroDesc."</option>";
		}	
	...
	if (mysql_num_rows($resultadok) > 0)
			...
			while($fila=mysql_fetch_array($resultadok))
				...
				if($_GET['idmanga']==$idManga2)
					...
					echo "<option value='manga_newb.php?id=".$idCarrera."&idetapa=".$ideta."&idmanga=".$idManga2."&idseccion=".$idsec."&newBD=true&campeonato=".$_GET['campeonato']."&copa=".$_GET['copa']."'>".$desc."</option>";
					}
				}
			}
...
echo "<hr><br>";
				if($tipo_prueba==2)
						...
						include("mangas_recarga_newb.php");
					?>
					</div>
								<?php
include("visitas.php");
...
<!-- Link back to Colorlib can't be removed. Template is licensed under CC BY 3.0. -->
	...
	<?php
    // Ejecutar esta funcion al final del documento a cachear
    ob_end_flush();
?>