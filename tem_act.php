...
$dbQuery = "SELECT idcarreras, descripcion, fecha_larga, organizador, fecha, idWeb, titulo, poblacion FROM carreras WHERE (desactivada = '0' OR desactivada IS NULL) AND (idGrupo = '0' OR idGrupo IS NULL OR idGrupo IN (SELECT idGrupo FROM pruebas_especiales)) AND temporada = '".$anoActual."' ORDER BY fecha DESC";
$resultado = mysql_query($dbQuery) or print "No se pudo acceder al contenido de las pruebas online.";
			...
			echo "<tr><th>Fecha</th><th>Poblaci&oacute;n</th><th>Evento</th><th>Organizador</th></thead><tbody>";
...
$dbQuery = "SELECT idGrupo, titulo, fecha, fecha_larga, temporada, organizador, poblacion, desactiva FROM grupos WHERE (desactiva = '0' OR desactiva IS NULL) AND temporada = '".$anoActual."' ORDER BY fecha DESC";
$resultado = mysql_query($dbQuery) or print "No se pudo acceder al contenido de las pruebas online.";
if (mysql_num_rows($resultado) > 0){	
while($fila=mysql_fetch_array($resultado)){
	$fecha_larga = $fila["fecha_larga"];
...
for($i=0; $i<count($CarrerasyGrupos); $i++){
	...
	if($CarrerasyGrupos[$i]->idGrupo>0){//Este objeto carrera se ha creado de la información de un grupo
		if(isset($_GET['grupo']) && $CarrerasyGrupos[$i]->idGrupo==$_GET['grupo'])
			...
			//echo "<tr class='negrita'><td colspan='4' class='center'>Grupo de Eventos ".$CarrerasyGrupos[$i]->titulo.":</td></tr>";
				...
				if(mysql_num_rows($resultado) != 0){
					...
					$dbQuery = "SELECT idcarreras, descripcion, fecha_larga, organizador, fecha, idWeb, titulo, poblacion FROM carreras WHERE (desactivada = '0' OR desactivada IS NULL) AND temporada = '".$anoActual."' AND idGrupo = '".$grupo."' ORDER BY fecha DESC";
					$resultado = mysql_query($dbQuery) or print "No se pudo acceder al contenido de las pruebas online.";
					if (mysql_num_rows($resultado) == 0)
						...
						if($idCarrera==0 || $idCarrera=='0' || $idCarrera==NULL || $idCarrera==''){//Prueba creada a través de la WEB
								...
								echo '<div class="table-row">
											<div class="serial">'.$fecha_larga.'</div>
											<div class="country">'.$poblacion.'</div>
											<div class="visit">'.$titulo.'</div>
		...
		echo "<tr class='".$classcss."' onclick=\"location.href='prueba.php?grupo=".$grupo."&lang=".$_GET['lang']."'\"><td>".$CarrerasyGrupos[$i]->fecha_larga."</td><td>".$CarrerasyGrupos[$i]->poblacion."</td><td><a href='prueba.php?grupo=".$grupo."&lang=".$_GET['lang']."'>".nombresTildes($CarrerasyGrupos[$i]->titulo)."</a></td><td>".$CarrerasyGrupos[$i]->organizador."</td></tr>";
			}
	}
