...
if(isset($_GET['manga']))
	...
	veh.grupo,veh.clase AS clase,veh.marca,veh.modelo,com.hora_salida AS hora,car.id AS idcarrera
...
//FALTARA CONSULTAR EL ESTADO DE LA MANGA PARA SABER LOS RETIRADOS
	if(mysql_num_rows($resultado)>0)
		...
		while($fila=mysql_fetch_array($resultado))
			...
			$sql_copi = mysql_query("SELECT copi.nombre AS copiloto FROM abc_57os_ca_copiloto copi 
			...
			//$orden = $fila['orden'];
			$hora = $fila['hora'];
			$abandono = mysql_query("SELECT id FROM abc_57os_ca_abandono WHERE id_ca_carrera='$idCarrera' AND dorsal='$dorsal'");
...
echo "<table>";
echo "<tr><td class='amarillo cubos_leyenda'>".$porsalir."</td><td class='verde cubos_leyenda'>".$enpista."</td><td class='gris cubos_leyenda'>".$llegada."</td><td class='rojooo cubos_leyenda'>".$abandonados."</td><td class='azul cubos_leyenda'>0</td></tr>";
echo "</table>";
echo "<hr>";
//REPITO LA MATRIZ POR SI HAY QUE CAMBIAR LA FORMA DE MOSTRARLOS
		...
		foreach ($ordenar as $key => $row) {
			...
			if($row['tipo']==2){
			echo "<div class='cubos ".$color."' onclick=\"info('".$row['piloto']."','".$row['dorsal']."','".$row['vehiculo']."','".$row['modelo']."','".$row['copiloto']."')\"><p class='dorsaln'>".$row['dorsal']."</p>
				<p class='nomargen t1'>EN PISTA</p>
				<p class='nomargen t2'>Hs:".$row['t_s']."</p>";
				if ($num_inter>0)
					{
					$mi_cuenta = 1;
					echo "<p class='nomargen t2 centro'><hr>INTERMEDIOS<br>";
					while($mi_cuenta!=$num_inter+1)
							{
								$int='i'.$mi_cuenta;
									if($row[$int]>0)
										echo "<span class='tachado centro'>".$mi_cuenta."</span> ";
									else
										echo "<span class='frojo centro'>".$mi_cuenta."</span> ";
							$mi_cuenta++;
							}
					echo "</p>";
					}
		...
		foreach ($ordenar as $key => $row) {
			...
			$color = colores($row['tipo']);
			if($row['tipo']==1)
			echo "<div class='cubos ".$color."' onclick=\"info('".$row['piloto']."','".$row['dorsal']."','".$row['vehiculo']."','".$row['modelo']."','".$row['copiloto']."')\"><p class='dorsaln'>".$row['dorsal']."</p><p class='negrita'></p><p></p></div>";
			$pos++;
		}
		//META
		foreach ($ordenar as $key => $row) {
